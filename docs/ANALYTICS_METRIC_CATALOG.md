...
| `sales.pipeline_value`            | Valor comercial de oportunidades abiertas cubiertas. | `opportunities.amount/currency/status` | string decimal por moneda; unknown reduce cobertura. |
...
Las métricas de Opportunity permiten filtrar por `customerNeed`, `authorizedSolution` y
`authorizedProduct`. Referencias null de datos legacy permanecen unknown; nunca se derivan desde el
texto libre histórico `Prospect.interest`.
