...
describe('CrmService', () => {
  ...
  it('exige permiso específico para cerrar una oportunidad', async () => {
    const db = {
      opportunity: { findFirst: jest.fn().mockResolvedValue({ id:'opp', prospectId:'prospect', stageId:'old', status:'OPEN', stage:{key:'new'} }) },
      pipelineStage: { findFirst: jest.fn().mockResolvedValue({ id:'closed', key:'closed', name:'Cerrado' }) },
      $transaction: jest.fn(),
    } as any;
    const service = new CrmService(db, { record: jest.fn() } as any);
    await expect(service.moveOpportunity('opp',{stageId:'closed',outcome:'WON'},consultant,{auth:{user:consultant},headers:{}})).rejects.toThrow('No puede cerrar oportunidades');
    expect(db.$transaction).not.toHaveBeenCalled();
  });
