...
export class ProspectsService {
  ...
  async capture(input: CaptureProspectInput, context: AuditContext) {
    ...
    try {
      return await this.captureTransaction(input, normalizedEmail, normalizedPhone, context);
    } catch (error) {
      if (!(error instanceof Prisma.PrismaClientKnownRequestError) || error.code !== 'P2002') throw error;
      const idempotent = await this.db.leadEvent.findUnique({
        where: { submissionId: input.submissionId }, select: { prospect: { select: { id: true, status: true } } },
      });
      if (idempotent) return idempotent.prospect;
      return this.captureTransaction(input, normalizedEmail, normalizedPhone, context);
    }
  }

  private captureTransaction(input: CaptureProspectInput, normalizedEmail: string | undefined, normalizedPhone: string | undefined, context: AuditContext) {
    return this.db.$transaction(async tx => {
      ...
      const source = await tx.leadSource.findFirst({ where: { key: input.source, isActive: true } });
      ...
      const prospect = existing
        ...
        : await tx.prospect.create({ data: {
            name: input.name, phone: input.phone, normalizedPhone, email: input.email, normalizedEmail,
            city: input.city, sourceId: source.id, campaign: input.campaign, landing: input.landing,
            interest: input.interest, message: input.message,
          } });

      await tx.consent.create({ data: {
        prospectId: prospect.id, accepted: input.consent.accepted,
        privacyVersion: input.consent.privacyVersion, ipAddress: context.ipAddress,
        userAgent: context.userAgent,
      } });
      const eventType = existing ? LeadEventType.RECAPTURED : LeadEventType.CAPTURED;
      await tx.leadEvent.create({ data: {
        prospectId: prospect.id, submissionId: input.submissionId, type: eventType,
        sourceId: source.id, campaign: input.campaign, landing: input.landing, interest: input.interest,
      } });
      await this.audit.record('PROSPECT_CAPTURED', 'Prospect', prospect.id, context, {
        source: input.source, landing: input.landing, eventType,
      }, tx);
      console.info(JSON.stringify({ level: 'info', event: 'prospect_captured', prospectId: prospect.id, source: input.source, landing: input.landing, duplicate: Boolean(existing) }));
  ...
  async list(query: ProspectListQuery) {
    ...
    const [data, total] = await this.db.$transaction([
      this.db.prospect.findMany({
        ...
        select: {
          id: true, name: true, email: true, phone: true, city: true, status: true,
          landing: true, interest: true, campaign: true, firstCapturedAt: true, lastCapturedAt: true,
