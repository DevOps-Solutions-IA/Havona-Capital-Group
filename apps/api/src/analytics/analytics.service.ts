...
const complete = (covered: number, total = covered): AnalyticsCoverage => ({
  status: 'COMPLETE',
  covered,
  total,
  percentage: total ? Math.round((covered / total) * 10000) / 100 : 100,
});
const unavailable = (warning: string): AnalyticsCoverage => ({
...
export class AnalyticsService {
  ...
  catalog() {
    return { version: 1, timezoneDefault: 'America/Bogota', definitions: METRIC_CATALOG };
  }

  async scope(actor: AnalyticsActor, requestedUserId?: string): Promise<AnalyticsScope> {
    if (actor.permissions.includes('analytics.read_all'))
      return requestedUserId ? { kind: 'GLOBAL', userIds: [requestedUserId] } : { kind: 'GLOBAL' };
    if (actor.permissions.includes('analytics.read_team')) {
      ...
      if (requestedUserId && !allowed.includes(requestedUserId))
        throw new ForbiddenException('El usuario no pertenece al ámbito analítico autorizado');
      return { kind: 'TEAM', userIds: requestedUserId ? [requestedUserId] : allowed };
    }
    if (requestedUserId && requestedUserId !== actor.id)
      throw new ForbiddenException('No puede consultar analítica de otro consultor');
    return { kind: 'OWN', userIds: [actor.id] };
  }

  private prospectWhere(scope: AnalyticsScope): Prisma.ProspectWhereInput {
    return scope.userIds
      ? { assignments: { some: { assigneeId: { in: scope.userIds }, endedAt: null } } }
      : {};
  }
  private opportunityWhere(scope: AnalyticsScope, query: any = {}): Prisma.OpportunityWhereInput {
    return {
      ...(scope.userIds ? { ownerId: { in: scope.userIds } } : {}),
      ...(query.customerNeedKey ? { customerNeed: { key: query.customerNeedKey } } : {}),
      ...(query.authorizedProductId ? { authorizedProductId: query.authorizedProductId } : {}),
      ...(query.authorizedSolutionId ? { authorizedSolutionId: query.authorizedSolutionId } : {}),
    };
  }
  private periodResult(period: AnalyticsPeriod) {
    return {
  ...
  async metric(key: string, query: any, actor: AnalyticsActor): Promise<MetricResult> {
    ...
    if (key === 'sales.pipeline_value' || key === 'sales.weighted_pipeline') {
      const rows = await this.db.opportunity.findMany({
        where: { ...this.opportunityWhere(scope, query), status: 'OPEN' },
      ...
      coverage = {
        ...
        ...(eligible.length < rows.length
          ? {
              warning:
                'Las oportunidades sin monto/moneda/probabilidad se excluyen; no equivalen a cero.',
            }
          : {}),
      };
      return {
    ...
    if (
      [
        'sales.won_value',
        'sales.lost_potential_value',
        'sales.average_deal_value',
        'sales.median_deal_value',
        'sales.commit_forecast',
      ].includes(key)
    ) {
      const status =
        key === 'sales.lost_potential_value'
          ? 'LOST'
          : key === 'sales.commit_forecast'
            ? 'OPEN'
            : 'WON';
      const rows = await this.db.opportunity.findMany({
        where: {
          ...this.opportunityWhere(scope, query),
          status,
          ...(status === 'OPEN'
      ...
      return {
        ...
        coverage: {
          ...
          percentage: rows.length ? Math.round((eligible.length / rows.length) * 10000) / 100 : 100,
          ...(eligible.length < rows.length
            ? { warning: 'Registros sin monto/moneda se excluyen y reducen cobertura.' }
            : {}),
        },
        source: definition.source,
        generatedAt: new Date().toISOString(),
    ...
    if (key === 'sales.financial_coverage') {
      ...
      coverage = total
        ...
        : {
            ...
            total: 0,
            percentage: null,
            warning: 'No existen oportunidades abiertas.',
          };
    } else if (key === 'crm.prospects.created')
      value = await this.db.prospect.count({
        where: { ...this.prospectWhere(scope), createdAt: range },
      });
    else if (key === 'crm.prospects.assigned')
      value = await this.db.assignment.count({
        where: {
          ...(scope.userIds ? { assigneeId: { in: scope.userIds } } : {}),
          createdAt: range,
        },
      });
    else if (key === 'crm.opportunities.created')
      value = await this.db.opportunity.count({
        where: { ...this.opportunityWhere(scope, query), createdAt: range },
      });
    else if (key === 'crm.opportunities.won' || key === 'crm.opportunities.lost')
      value = await this.db.opportunity.count({
        where: {
          ...this.opportunityWhere(scope, query),
          status: key.endsWith('won') ? 'WON' : 'LOST',
          closedAt: range,
        },
      });
      ...
      [numerator, denominator] = await Promise.all([
        this.db.opportunity.count({
          where: { ...this.opportunityWhere(scope, query), status: 'WON', closedAt: range },
        }),
        this.db.opportunity.count({
          where: {
            ...this.opportunityWhere(scope, query),
      ...
      coverage = denominator
        ...
        : {
            ...
            total: 0,
            percentage: null,
            warning: 'No existen cierres comparables en el periodo.',
          };
      ...
      const days = rows.flatMap((row) =>
        row.closedAt ? [(row.closedAt.getTime() - row.createdAt.getTime()) / 86_400_000] : [],
      );
      ...
      coverage = days.length
        ...
        : {
            ...
            total: 0,
            percentage: null,
            warning: 'No existen oportunidades cerradas.',
          };
    } else if (key === 'activities.interactions')
      value = await this.db.interaction.count({
        where: { ...(scope.userIds ? { actorId: { in: scope.userIds } } : {}), occurredAt: range },
      });
    else if (key === 'activities.tasks_completed')
      value = await this.db.task.count({
        where: {
          ...(scope.userIds ? { assigneeId: { in: scope.userIds } } : {}),
          completedAt: range,
        },
      });
    else if (key === 'activities.tasks_overdue')
      value = await this.db.task.count({
        where: {
          ...(scope.userIds ? { assigneeId: { in: scope.userIds } } : {}),
    ...
    else if (key === 'calendar.appointments_scheduled')
      value = await this.db.calendarEventLink.count({
        where: {
          ...(scope.userIds ? { assignedConsultantId: { in: scope.userIds } } : {}),
          startAt: range,
        },
      });
    else if (key === 'communications.inbound' || key === 'communications.outbound')
      value = await this.db.communicationMessage.count({
        where: {
          direction: key.endsWith('inbound') ? 'INBOUND' : 'OUTBOUND',
          createdAt: range,
          ...(scope.userIds ? { thread: { assignedUserId: { in: scope.userIds } } } : {}),
        },
      });
    else if (key === 'automations.executions')
      value = await this.db.automationExecution.count({
        where: {
          createdAt: range,
          ...(scope.userIds ? { workflow: { ownerUserId: { in: scope.userIds } } } : {}),
        },
      });
    else if (key === 'cadences.enrollments')
      value = await this.db.cadenceEnrollment.count({
        where: {
          createdAt: range,
          ...(scope.userIds ? { enrolledById: { in: scope.userIds } } : {}),
        },
      });
    else if (key === 'cadences.completed')
      value = await this.db.cadenceEnrollment.count({
        where: {
          status: 'COMPLETED',
          stoppedAt: range,
    ...
    else if (key === 'cadences.stopped_by_reply')
      value = await this.db.cadenceEnrollment.count({
        where: {
    ...
    else if (key === 'henry.conversations')
      value = await this.db.conversation.count({
        where: {
          createdAt: range,
          ...(scope.userIds ? { participants: { some: { userId: { in: scope.userIds } } } } : {}),
        },
      });
    else if (key === 'henry.escalations')
      value = await this.db.escalation.count({
        where: {
          createdAt: range,
    ...
    return {
      ...
      version: definition.version,
      value,
      availability: value === null ? 'notAvailable' : 'available',
      numerator,
      denominator,
      period: this.periodResult(period),
      scope: scope.kind,
  ...
  async compareMetric(key: string, query: any, actor: AnalyticsActor) {
    ...
    const current = await this.metric(
      key,
      {
        ...query,
        preset: 'custom',
    ...
    return {
      ...
      delta: {
        absolute,
        percentage,
        status: !comparable || prior.value === 0 ? 'NOT_COMPARABLE' : 'COMPARABLE',
      },
    };
  }
  ...
  async summary(query: any, actor: AnalyticsActor) {
    ...
    const [funnel, pipeline, priorities, goals, quality] = await Promise.all([
      this.funnel(query, actor),
      this.pipeline(query, actor),
      this.priorities(query, actor),
      this.goals(query, actor),
      this.dataQuality(query, actor),
    ]);
    return {
      period: metrics[0]?.current.period,
      freshness: { status: 'LIVE', generatedAt: new Date().toISOString(), expectationMinutes: 5 },
      metrics,
      funnel,
      pipeline,
  ...
  async funnel(query: any, actor: AnalyticsActor) {
    ...
    const histories = await this.db.opportunityStageHistory.findMany({
      where: { createdAt: range, opportunity: this.opportunityWhere(scope, query) },
      select: { opportunityId: true, newStageId: true, createdAt: true },
    });
    const entered = new Map<string, Set<string>>();
    histories.forEach((item) => {
      const set = entered.get(item.newStageId) ?? new Set<string>();
      set.add(item.opportunityId);
      entered.set(item.newStageId, set);
    });
    const durationRows = await this.db.opportunityStageHistory.findMany({
      where: { opportunity: this.opportunityWhere(scope, query) },
      orderBy: [{ opportunityId: 'asc' }, { createdAt: 'asc' }],
      select: { opportunityId: true, newStageId: true, createdAt: true },
    });
    const durations = new Map<string, number[]>();
    for (let index = 0; index < durationRows.length - 1; index++) {
      const current = durationRows[index]!,
        next = durationRows[index + 1]!;
      if (current.opportunityId !== next.opportunityId) continue;
      const list = durations.get(current.newStageId) ?? [];
      list.push((next.createdAt.getTime() - current.createdAt.getTime()) / 86_400_000);
      durations.set(current.newStageId, list);
    }
    const rows = stages.map((stage, index) => {
      const count = entered.get(stage.id)?.size ?? 0;
      const nextStage = stages[index + 1];
      const nextCount = nextStage ? (entered.get(nextStage.id)?.size ?? 0) : null;
      const times = durations.get(stage.id) ?? [];
      return {
        stage: { id: stage.id, key: stage.key, name: stage.name, position: stage.position },
        entered: count,
        conversionToNext: nextCount === null || count === 0 ? null : Math.min(1, nextCount / count),
        averageDays: times.length ? times.reduce((a, b) => a + b, 0) / times.length : null,
    ...
    return {
      ...
      scope: scope.kind,
      semantics:
        'Cada oportunidad cuenta una vez por etapa dentro del periodo aunque reingrese. Duración usa transiciones consecutivas históricas.',
      stages: rows,
      leakage: rows.slice(0, -1).map((row, i) => ({
        from: row.stage.key,
        to: rows[i + 1]!.stage.key,
        count: Math.max(0, row.entered - rows[i + 1]!.entered),
      })),
    };
  }
  ...
  async pipeline(query: any, actor: AnalyticsActor) {
    ...
    const rows = await this.db.opportunity.findMany({
      ...
      include: {
        ...
        customerNeed: { select: { key: true, name: true } },
        authorizedSolution: { select: { id: true, key: true, name: true } },
        authorizedProduct: { select: { id: true, key: true, name: true, carrier: true } },
        tasks: {
    ...
    const items = rows.map((row) => {
      const inactivityDays = Math.floor(
        (now.getTime() - (row.interactions[0]?.occurredAt ?? row.createdAt).getTime()) / 86_400_000,
      );
      const overdue = row.tasks.some((task) => task.dueAt < now);
      const components = [
        {
          factor: 'INACTIVITY',
          contribution: inactivityDays >= 14 ? 25 : inactivityDays >= 7 ? 12 : 0,
          evidence: `${inactivityDays} días sin interacción`,
        },
        {
          factor: 'OVERDUE_TASK',
          contribution: overdue ? 20 : 0,
          evidence: overdue ? 'Existe tarea vencida' : 'Sin tarea vencida',
        },
        {
          factor: 'NO_NEXT_ACTION',
          contribution: row.tasks.length ? 0 : 15,
          evidence: row.tasks.length ? 'Existe siguiente tarea' : 'No existe siguiente tarea',
        },
      ].filter((item) => item.contribution);
      const riskScore = components.reduce((sum, item) => sum + item.contribution, 0);
      return {
        id: row.id,
        title: row.title,
        stage: row.stage,
        ...
        authorizedSolution: row.authorizedSolution,
        authorizedProduct: row.authorizedProduct,
        inactivityDays,
        riskScore,
        ...
        money:
          row.amount && row.currency
            ? { amount: row.amount.toFixed(2), currency: row.currency }
            : null,
    ...
    const stageValue = Array.from(
      rows.reduce((groups, row) => {
        ...
        if (row.amount && row.currency)
          current.money.push({ currency: row.currency, amount: row.amount });
    ...
    return {
      ...
      monetaryValue: {
        ...
        coverage: {
          ...
          percentage: rows.length
            ? Math.round((withAmount.length / rows.length) * 10000) / 100
            : 100,
          ...(withAmount.length < rows.length
            ? { warning: 'Monto desconocido no se convierte en cero.' }
            : {}),
        },
      },
      weightedPipeline: {
        ...
        coverage: {
          ...
          ...(weighted.length < rows.length
            ? { warning: 'Probability desconocida se excluye, no se pondera como cero.' }
            : {}),
        },
      },
      forecast: {
        period: this.periodResult(period),
        categories: forecast,
        label: 'DETERMINISTIC_NOT_GUARANTEED',
      },
      stageValue,
      ...
      methodology: {
        version: 1,
        maximumObservedScore: 60,
        warning: 'Risk score explicable; no representa probabilidad de pérdida.',
      },
    };
  }

  async priorities(query: any, actor: AnalyticsActor) {
    const pipeline = await this.pipeline(query, actor);
    const scope = await this.scope(actor, query.consultantId);
    const now = new Date();
    ...
    return [
      ...
      ...pipeline.aging
        .filter(
          (item: any) =>
            item.expectedCloseDate && item.expectedCloseDate < now.toISOString().slice(0, 10),
        )
        .map((item: any) => ({
      ...
      ...tasks.map((task) => ({
        ...
        entityId: task.id,
        title: task.title,
        reason: `Vencida desde ${task.dueAt.toISOString()}`,
        evidence: [{ dueAt: task.dueAt, assignee: task.assignee }],
        suggestedAction: 'COMPLETE_OR_RESCHEDULE_TASK',
        urgency: 50,
      })),
  ...
  async cohorts(query: any, actor: AnalyticsActor) {
    ...
    const prospects = await this.db.prospect.findMany({
      ...
      select: {
        id: true,
        createdAt: true,
        opportunities: { select: { createdAt: true, status: true, closedAt: true } },
      },
    ...
    const cohorts = new Map<
      ...
      {
        created: number;
        opportunitiesWithin30Days: number;
        wonWithin90Days: number;
        mature30: number;
        mature90: number;
      }
    >();
    ...
    for (const prospect of prospects) {
      ...
      if (
        prospect.opportunities.some(
          (item) => item.createdAt.getTime() - prospect.createdAt.getTime() <= 30 * 86_400_000,
        )
      )
        row.opportunitiesWithin30Days += 1;
      if (
        prospect.opportunities.some(
          (item) =>
            item.status === 'WON' &&
            item.closedAt &&
            item.closedAt.getTime() - prospect.createdAt.getTime() <= 90 * 86_400_000,
        )
      )
        row.wonWithin90Days += 1;
      cohorts.set(key, row);
    }
    return {
      period: this.periodResult(period),
      scope: scope.kind,
      cohortBasis: 'prospect.createdAt',
      rows: [...cohorts].map(([cohort, row]) => ({
        cohort,
        ...row,
        opportunityConversion30Days: row.mature30
          ? row.opportunitiesWithin30Days / row.mature30
          : null,
        wonConversion90Days: row.mature90 ? row.wonWithin90Days / row.mature90 : null,
        maturity:
          row.mature90 === row.created
            ? 'MATURE_90'
            : row.mature30 === row.created
              ? 'MATURE_30'
              : 'INCOMPLETE',
      })),
      warning:
        'Las cohortes inmaduras se identifican y no se comparan como si hubieran completado su ventana.',
    };
  }

  async dataQuality(query: any, actor: AnalyticsActor) {
    const scope = await this.scope(actor, query.consultantId),
      opportunityWhere = this.opportunityWhere(scope, query),
    ...
    const [
      ...
      this.db.opportunity.count({
        where: { ...opportunityWhere, status: 'OPEN', OR: [{ amount: null }, { currency: null }] },
      }),
      this.db.opportunity.count({
        where: { ...opportunityWhere, status: 'OPEN', expectedCloseDate: null },
      }),
      this.db.opportunity.count({
        where: { ...opportunityWhere, status: 'OPEN', probability: null },
      }),
      this.db.opportunity.count({
        where: {
          ...opportunityWhere,
          ...
          OR: [{ amount: null }, { currency: null }, { expectedCloseDate: null }],
        },
      }),
    ]);
    ...
    const checks = [
      ...
      },
      {
        ...
        total: openOpportunities,
        note: 'Sin política por etapa: probability permanece null salvo captura manual.',
      },
      {
        key: 'opportunity.commit_completeness',
        missing: commitWithoutAmount,
        total: openOpportunities,
        note: 'COMMIT requiere monto, moneda y fecha para forecast.',
      },
    ].map((item) => ({
      ...item,
      coveragePercentage: item.total
        ? Math.round(((item.total - item.missing) / item.total) * 10000) / 100
        : null,
    }));
    return {
      summary: {
        status: checks.some((item) => item.missing > 0) ? 'PARTIAL' : 'COMPLETE',
        checks: checks.length,
        issues: checks.reduce((sum, item) => sum + item.missing, 0),
      },
      checks,
      unknownIsZero: false,
    };
  ...
  async communications(query: any, actor: AnalyticsActor) {
    ...
    return {
      ...
      satisfaction: { value: null, availability: 'notAvailable' },
    };
  }
  ...
  async automations(query: any, actor: AnalyticsActor) {
    ...
    const [total, completed, failed, approvals] = await Promise.all([
      this.db.automationExecution.count({ where }),
      this.db.automationExecution.count({ where: { ...where, status: 'COMPLETED' } }),
      this.db.automationExecution.count({ where: { ...where, status: 'FAILED' } }),
      this.db.automationApproval.count({ where: { status: 'PENDING', execution: where } }),
    ]);
    return {
      period: this.periodResult(period),
      executions: total,
      ...
      attributionWarning: 'Interacción con automatización no implica causalidad comercial.',
    };
  }
  ...
  async henry(query: any, actor: AnalyticsActor) {
    const period = analyticsPeriod(query),
      scope = await this.scope(actor, query.consultantId),
      range = { gte: period.start, lt: period.end },
      conversationScope = scope.userIds
        ? { participants: { some: { userId: { in: scope.userIds } } } }
        : {};
    const [conversations, escalations, executions, failures, usage] = await Promise.all([
      this.db.conversation.count({ where: { ...conversationScope, createdAt: range } }),
      ...
      this.db.aIExecution.count({ where: { startedAt: range, conversation: conversationScope } }),
      this.db.aIExecution.count({
        where: { startedAt: range, status: 'FAILED', conversation: conversationScope },
      }),
      this.db.aIUsage.aggregate({
        where: { execution: { startedAt: range, conversation: conversationScope } },
    ...
    return {
      ...
      executions,
      failures,
      resolutionWithoutEscalation: conversations
        ? Math.max(0, conversations - escalations) / conversations
        : null,
      usage: {
        inputTokens: usage._sum.inputTokens ?? 0,
        outputTokens: usage._sum.outputTokens ?? 0,
        costUsd: usage._sum.estimatedCostUsd?.toNumber() ?? null,
        costLabel:
          usage._sum.estimatedCostUsd == null
            ? 'notAvailable'
            : 'provider-reported-or-estimated-as-stored',
      },
    };
  }
  ...
  async goals(query: any, actor: AnalyticsActor) {
    const scope = await this.scope(actor, query.consultantId);
    const rows = await this.db.analyticsGoal.findMany({
      where: {
        ...(scope.userIds
          ? {
              OR: [
                { scopeUserId: { in: scope.userIds } },
                {
                  scopeType: 'ORGANIZATION',
                  ...(scope.kind === 'OWN' ? { scopeUserId: null } : {}),
                },
              ],
            }
          : {}),
    ...
    return Promise.all(
      rows.map(async (row) => {
          ...
          effectiveEnd > row.periodStart
            ? await this.metric(
                row.metricKey,
                {
                  ...
                  consultantId: row.scopeUserId ?? query.consultantId,
                },
                actor,
              )
            : null;
        ...
        const elapsed = Math.max(
          ...
          Math.min(
            1,
            (effectiveEnd.getTime() - row.periodStart.getTime()) /
        ...
        return {
          ...
          progress: {
            ...
            remaining:
              actual === null
                ? null
                : Prisma.Decimal.max(0, target.sub(actual)).toFixed(row.currency ? 2 : 4),
            elapsedPeriod: elapsed,
            linearPaceExpected: target.mul(elapsed).toFixed(row.currency ? 2 : 4),
            gapToPace:
              actual === null
                ? null
                : actual.sub(target.mul(elapsed)).toFixed(row.currency ? 2 : 4),
  ...
  async createGoal(input: any, actor: AnalyticsActor, request: any) {
    ...
    if (input.scopeType !== 'ORGANIZATION' && !input.scopeUserId)
      throw new BadRequestException('La meta de equipo o usuario requiere un ámbito explícito');
    if (input.scopeUserId) await this.scope(actor, input.scopeUserId);
    const row = await this.db.analyticsGoal.create({
      data: {
    ...
    await this.audit.record(
      'ANALYTICS_GOAL_CREATED',
      'AnalyticsGoal',
      row.id,
      { actorUserId: actor.id, ipAddress: request.ip, userAgent: request.headers?.['user-agent'] },
      { metricKey: row.metricKey, scopeType: row.scopeType },
    );
    return row;
  }
  async updateGoal(id: string, input: any, actor: AnalyticsActor, request: any) {
    ...
    if (
      ...
      typeof input.targetValue !== 'string'
    )
      throw new BadRequestException('La meta monetaria debe enviarse como string decimal');
  ...
  async team(query: any, actor: AnalyticsActor) {
    const scope = await this.scope(actor);
    if (scope.kind === 'OWN') throw new ForbiddenException('Sin acceso a analítica de equipo');
    const ids =
      scope.userIds ??
      (await this.db.user.findMany({ where: { isActive: true }, select: { id: true } })).map(
        (u) => u.id,
      );
  ...
  async anomalies(query: any, actor: AnalyticsActor) {
    const current = await this.compareMetric('activities.tasks_overdue', query, actor);
    if (
      current.current.value === null ||
      current.previous.value === null ||
      current.previous.value < 5
    )
      return {
        data: [],
        status: 'INSUFFICIENT_DATA',
        reason: 'Se requieren al menos 5 observaciones en el periodo base.',
      };
    const ratio = current.current.value / current.previous.value;
    return {
      data:
        ratio >= 1.5
          ? [
              {
                metric: 'activities.tasks_overdue',
                observed: current.current.value,
                baseline: current.previous.value,
                magnitude: ratio,
                confidence: 'RULE_BASED',
                explanation: 'Incremento igual o superior a 50% frente al periodo equivalente.',
              },
            ]
          : [],
      status: 'AVAILABLE',
