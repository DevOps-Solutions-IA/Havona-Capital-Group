...
export class CalendarConfig {
  ...
  readonly redirectUri = process.env.GOOGLE_CALENDAR_REDIRECT_URI?.trim() ?? 'http://localhost:3001/api/v1/integrations/google/calendar/oauth/callback';
  readonly baseUrl = (process.env.GOOGLE_CALENDAR_BASE_URL?.trim() || 'https://www.googleapis.com/calendar/v3').replace(/\/$/, '');
  ...
  readonly allowMeet = process.env.GOOGLE_CALENDAR_ALLOW_GOOGLE_MEET !== 'false';
  readonly timeoutMs = Number(process.env.GOOGLE_CALENDAR_REQUEST_TIMEOUT_MS ?? 15_000);
  readonly scopes = (process.env.GOOGLE_CALENDAR_SCOPES?.trim() || [
  ...
  assertConfigured() {
    if (!this.enabled || !this.clientId || !this.clientSecret || !this.encryptionKey) throw new Error('CALENDAR_CONFIGURATION_REQUIRED');
  }
}
