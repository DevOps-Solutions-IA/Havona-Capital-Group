...
export class CalendarService {
  ...
  async beginOAuth(actor: Actor) {
    ...
    await this.db.calendarOAuthState.create({ data: { stateHash: hash(state), userId: actor.id, encryptedCodeVerifier: this.vault.encrypt(verifier), expiresAt: new Date(Date.now() + 10 * 60_000) } });
    const query = new URLSearchParams({ client_id: this.config.clientId, redirect_uri: this.config.redirectUri, response_type: 'code',
      scope: this.config.scopes.join(' '), access_type: 'offline', include_granted_scopes: 'true', prompt: 'consent', state,
  ...
  async completeOAuth(code: string, state: string, audit: AuditContext) {
    this.config.assertConfigured();
    const record = await this.db.calendarOAuthState.findUnique({ where: { stateHash: hash(state) } });
    if (!record || record.consumedAt || record.expiresAt < new Date()) throw new CalendarError('CALENDAR_AUTH_STATE_INVALID', 'La autorización expiró o no es válida', 400);
    await this.db.calendarOAuthState.update({ where: { id: record.id }, data: { consumedAt: new Date() } });
    const tokens = await this.tokenRequest({ code, code_verifier: this.vault.decrypt(record.encryptedCodeVerifier), grant_type: 'authorization_code', redirect_uri: this.config.redirectUri });
    const calendars = await this.provider.getCalendars({ accessToken: tokens.access_token });
    const primary = calendars.find((item) => item.primary) ?? calendars[0];
    if (!primary) throw new CalendarError('CALENDAR_PERMISSION_DENIED', 'Google no devolvió un calendario accesible', 403);
    const connection = await this.db.calendarConnection.upsert({
      where: { provider_providerAccountId_userId: { provider: 'GOOGLE', providerAccountId: primary.id, userId: record.userId } },
      update: { accountEmail: primary.id, selectedCalendarId: primary.id, selectedCalendarName: primary.name, timezone: primary.timezone ?? this.config.timezone,
        encryptedAccessToken: this.vault.encrypt(tokens.access_token), ...(tokens.refresh_token ? { encryptedRefreshToken: this.vault.encrypt(tokens.refresh_token) } : {}),
        accessTokenExpiresAt: new Date(Date.now() + Number(tokens.expires_in ?? 3600) * 1000), grantedScopes: String(tokens.scope ?? this.config.scopes.join(' ')).split(' '), status: 'ACTIVE', disconnectedAt: null, lastErrorCode: null },
      create: { userId: record.userId, providerAccountId: primary.id, accountEmail: primary.id, selectedCalendarId: primary.id, selectedCalendarName: primary.name,
        timezone: primary.timezone ?? this.config.timezone, encryptedAccessToken: this.vault.encrypt(tokens.access_token), encryptedRefreshToken: tokens.refresh_token ? this.vault.encrypt(tokens.refresh_token) : null,
        accessTokenExpiresAt: new Date(Date.now() + Number(tokens.expires_in ?? 3600) * 1000), grantedScopes: String(tokens.scope ?? this.config.scopes.join(' ')).split(' ') },
    });
    await this.db.calendarAvailabilityRule.upsert({ where: { userId: record.userId }, update: {}, create: { userId: record.userId, ...(await this.defaultRules()), timezone: connection.timezone } });
    await this.audit.record('CALENDAR_CONNECTED', 'CalendarConnection', connection.id, { ...audit, actorUserId: record.userId }, { provider: 'GOOGLE', scopes: connection.grantedScopes });
  ...
  async status(actor: Actor) {
    ...
    return { id: connection.id, provider: connection.provider, status: connection.status, accountEmail: connection.accountEmail,
      calendarId: connection.selectedCalendarId, calendarName: connection.selectedCalendarName, timezone: connection.timezone,
      lastSyncedAt: connection.lastSyncedAt, enabled: this.config.enabled };
  }

  async calendars(actor: Actor) { const connection = await this.requireConnection(actor.id); return this.provider.getCalendars(await this.credentials(connection)); }
  async selectCalendar(actor: Actor, calendarId: string, ctx: AuditContext) {
    const connection = await this.requireConnection(actor.id); const calendars = await this.provider.getCalendars(await this.credentials(connection));
    const selected = calendars.find((item) => item.id === calendarId); if (!selected) throw new NotFoundException('Calendario no disponible');
    const updated = await this.db.calendarConnection.update({ where: { id: connection.id }, data: { selectedCalendarId: selected.id, selectedCalendarName: selected.name, timezone: selected.timezone ?? connection.timezone } });
    await this.db.calendarSyncState.deleteMany({ where: { connectionId: connection.id } });
    await this.audit.record('CALENDAR_SELECTED', 'CalendarConnection', connection.id, ctx, { calendarId: selected.id }); return this.status(actor);
  }

  async disconnect(actor: Actor, ctx: AuditContext) {
    const connection = await this.requireConnection(actor.id); const credentials = await this.credentials(connection);
    const channels = await this.db.calendarWebhookChannel.findMany({ where: { connectionId: connection.id, stoppedAt: null } });
    for (const channel of channels) if (channel.resourceId) await this.provider.stopWatch(credentials, channel.channelId, channel.resourceId).catch(() => undefined);
    const refresh = connection.encryptedRefreshToken ? this.vault.decrypt(connection.encryptedRefreshToken) : undefined;
    const access = this.vault.decrypt(connection.encryptedAccessToken);
    await fetch(`https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(refresh ?? access)}`, { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' } }).catch(() => undefined);
    await this.db.$transaction([this.db.calendarWebhookChannel.updateMany({ where: { connectionId: connection.id, stoppedAt: null }, data: { stoppedAt: new Date() } }),
      this.db.calendarConnection.update({ where: { id: connection.id }, data: { encryptedAccessToken: '', encryptedRefreshToken: null, status: 'DISCONNECTED', disconnectedAt: new Date() } })]);
    await this.audit.record('CALENDAR_DISCONNECTED', 'CalendarConnection', connection.id, ctx); return { status: 'DISCONNECTED' };
  }

  async getRules(actor: Actor) { return this.db.calendarAvailabilityRule.upsert({ where: { userId: actor.id }, update: {}, create: { userId: actor.id, ...(await this.defaultRules()) } }); }
  async updateRules(actor: Actor, input: any, ctx: AuditContext) {
    this.assertTimezone(input.timezone); const start = this.clock(input.workStart), end = this.clock(input.workEnd); if (start >= end) throw new BadRequestException('El horario de inicio debe ser anterior al final');
    const rule = await this.db.calendarAvailabilityRule.upsert({ where: { userId: actor.id }, update: input, create: { userId: actor.id, ...input } });
    await this.audit.record('CALENDAR_RULES_UPDATED', 'CalendarAvailabilityRule', rule.id, ctx, { fields: Object.keys(input) }); return rule;
  }
  ...
  private async availabilityForUser(actor: Actor, userId: string, input: { timeMin: string; timeMax: string; timezone: string; durationMinutes: number }, ignoredBusy?: { start: Date; end: Date }) {
    ...
    if (ignoredBusy) busy = busy.filter((range) => new Date(range.start).getTime() !== ignoredBusy.start.getTime() || new Date(range.end).getTime() !== ignoredBusy.end.getTime());
    const slots = this.findSlots(min, max, input.durationMinutes, rules, busy);
    return { timezone: input.timezone, durationMinutes: input.durationMinutes, slots, source: 'GOOGLE_FREEBUSY' };
  }
  ...
  async sync(actor: Actor) { const connection = await this.requireConnection(actor.id); return this.syncConnection(connection); }
  async startWatch(actor: Actor) {
    if (!this.config.webhookUrl || !this.config.webhookUrl.startsWith('https://') || !this.config.webhookSecret) throw new CalendarError('CALENDAR_CONFIGURATION_REQUIRED', 'Configure una URL HTTPS y secreto para activar notificaciones push', 409);
    const connection = await this.requireConnection(actor.id); const credentials = await this.credentials(connection); const previous = await this.db.calendarWebhookChannel.findMany({ where: { connectionId: connection.id, stoppedAt: null } }); const channelId = randomUUID(); const token = `${this.config.webhookSecret}.${randomBytes(24).toString('base64url')}`; const expiration = DateTime.utc().plus({ days: 6 }).toISO()!;
    const watch = await this.provider.watchEvents(credentials, { calendarId: connection.selectedCalendarId, channelId, token, address: this.config.webhookUrl, expiration });
    await this.db.calendarWebhookChannel.create({ data: { connectionId: connection.id, channelId: watch.channelId, resourceId: watch.resourceId, encryptedToken: this.vault.encrypt(token), expiration: new Date(watch.expiration) } });
    for (const item of previous) { if (item.resourceId) await this.provider.stopWatch(credentials, item.channelId, item.resourceId).catch(() => undefined); await this.db.calendarWebhookChannel.update({ where: { id: item.id }, data: { stoppedAt: new Date() } }); }
    return { active: true, expiration: watch.expiration };
  }
  async stopWatch(actor: Actor) { const connection = await this.requireConnection(actor.id); const channels = await this.db.calendarWebhookChannel.findMany({ where: { connectionId: connection.id, stoppedAt: null } }); const credentials = await this.credentials(connection); for (const item of channels) { if (item.resourceId) await this.provider.stopWatch(credentials, item.channelId, item.resourceId); await this.db.calendarWebhookChannel.update({ where: { id: item.id }, data: { stoppedAt: new Date() } }); } return { active: false }; }
  async webhook(headers: Record<string, string | string[] | undefined>) {
    const channelId = String(headers['x-goog-channel-id'] ?? ''); const resourceId = String(headers['x-goog-resource-id'] ?? ''); const token = String(headers['x-goog-channel-token'] ?? '');
    const channel = await this.db.calendarWebhookChannel.findUnique({ where: { channelId }, include: { connection: true } });
    if (!channel || channel.stoppedAt || channel.resourceId !== resourceId || !safeEqual(this.vault.decrypt(channel.encryptedToken), token)) throw new CalendarError('CALENDAR_WEBHOOK_INVALID', 'Notificación de calendario inválida', 403);
    await this.db.calendarWebhookChannel.update({ where: { id: channel.id }, data: { lastNotificationAt: new Date() } }); await this.syncConnection(channel.connection); return { accepted: true };
  }

  private async syncConnection(connection: any): Promise<{ synced: boolean; full: boolean }> {
    const credentials = await this.credentials(connection); const state = await this.db.calendarSyncState.findUnique({ where: { connectionId: connection.id } });
    let token = state?.syncToken ?? undefined, full = !token, pageToken: string | undefined, lastSyncToken: string | undefined;
    try {
      do { const page = await this.provider.listEvents(credentials, { calendarId: connection.selectedCalendarId, syncToken: token, pageToken, ...(!token ? { timeMin: DateTime.utc().minus({ months: 1 }).toISO()! } : {}) });
        for (const event of page.events) await this.upsertSyncedEvent(connection, event); pageToken = page.nextPageToken; lastSyncToken = page.nextSyncToken ?? lastSyncToken; } while (pageToken);
    } catch (error) { if (error instanceof CalendarError && error.code === 'CALENDAR_SYNC_TOKEN_EXPIRED') { await this.db.calendarSyncState.deleteMany({ where: { connectionId: connection.id } }); return this.syncConnection(connection); } throw error; }
    await this.db.calendarSyncState.upsert({ where: { connectionId: connection.id }, update: { syncToken: lastSyncToken, lastSyncAt: new Date(), lastStatus: 'SUCCEEDED', lastErrorCode: null, ...(full ? { lastFullSyncAt: new Date() } : {}) }, create: { connectionId: connection.id, syncToken: lastSyncToken, lastSyncAt: new Date(), lastFullSyncAt: new Date(), lastStatus: 'SUCCEEDED' } });
    await this.db.calendarConnection.update({ where: { id: connection.id }, data: { lastSyncedAt: new Date() } }); return { synced: true, full };
  }
  ...
  private findSlots(min: DateTime, max: DateTime, duration: number, rules: any, busy: Array<{ start: string; end: string }>) { const zone = rules.timezone; const now = DateTime.utc().plus({ minutes: rules.minimumNoticeMinutes }); const occupied = busy.map((x) => Interval.fromDateTimes(DateTime.fromISO(x.start).minus({ minutes: rules.bufferBeforeMinutes }), DateTime.fromISO(x.end).plus({ minutes: rules.bufferAfterMinutes })));
    ...
    while (day <= last && slots.length < 50) { if (rules.workingDays.includes(day.weekday)) { const [sh, sm] = rules.workStart.split(':').map(Number), [eh, em] = rules.workEnd.split(':').map(Number); let cursor = day.set({ hour: sh, minute: sm }); const close = day.set({ hour: eh, minute: em });
      while (cursor.plus({ minutes: duration }) <= close) { const end = cursor.plus({ minutes: duration }); const interval = Interval.fromDateTimes(cursor, end); if (cursor.toUTC() >= min.toUTC() && end.toUTC() <= max.toUTC() && cursor.toUTC() >= now && !occupied.some((x) => x.overlaps(interval))) slots.push({ start: cursor.toISO()!, end: end.toISO()! }); cursor = cursor.plus({ minutes: 15 }); } } day = day.plus({ days: 1 }); }
    return slots; }
  private assertEventInput(input: Partial<EventInput>) { if (!input.start || !input.end || !input.timezone) throw new BadRequestException('Inicio, fin y timezone son obligatorios'); this.assertTimezone(input.timezone); if (new Date(input.end) <= new Date(input.start)) throw new BadRequestException('Rango horario inválido'); }
  private assertTimezone(zone: string) { if (!IANAZone.isValidZone(zone)) throw new CalendarError('CALENDAR_INVALID_TIMEZONE', 'Zona horaria IANA inválida', 400); }
  private clock(value: string) { const [h = 0, m = 0] = value.split(':').map(Number); return h * 60 + m; }
  private async connection(userId: string, active = true) { return this.db.calendarConnection.findFirst({ where: { userId, ...(active ? { status: 'ACTIVE' } : {}) }, orderBy: { updatedAt: 'desc' } }); }
  private async requireConnection(userId: string) { const value = await this.connection(userId); if (!value) throw new CalendarError('CALENDAR_NOT_CONNECTED', 'Conecte Google Calendar para continuar', 409); return value; }
  private async credentials(connection: any) { if (connection.accessTokenExpiresAt && connection.accessTokenExpiresAt.getTime() < Date.now() + 60_000) return this.refresh(connection); return { accessToken: this.vault.decrypt(connection.encryptedAccessToken) }; }
  private async refresh(connection: any) { if (!connection.encryptedRefreshToken) { await this.db.calendarConnection.update({ where: { id: connection.id }, data: { status: 'NEEDS_REAUTHORIZATION', lastErrorCode: 'CALENDAR_AUTH_EXPIRED' } }); throw new CalendarError('CALENDAR_AUTH_EXPIRED', 'Google Calendar requiere nueva autorización', 401); }
    try { const token = await this.tokenRequest({ refresh_token: this.vault.decrypt(connection.encryptedRefreshToken), grant_type: 'refresh_token' }); await this.db.calendarConnection.update({ where: { id: connection.id }, data: { encryptedAccessToken: this.vault.encrypt(token.access_token), accessTokenExpiresAt: new Date(Date.now() + Number(token.expires_in ?? 3600) * 1000), status: 'ACTIVE', lastErrorCode: null } }); return { accessToken: token.access_token }; }
    catch (error) { await this.db.calendarConnection.update({ where: { id: connection.id }, data: { status: 'NEEDS_REAUTHORIZATION', lastErrorCode: 'CALENDAR_AUTH_EXPIRED' } }); throw error; } }
  private async tokenRequest(values: Record<string, string>) { const response = await fetch('https://oauth2.googleapis.com/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ client_id: this.config.clientId, client_secret: this.config.clientSecret, ...values }) }); const data = await response.json().catch(() => ({})) as any; if (!response.ok || !data.access_token) throw new CalendarError('CALENDAR_AUTH_EXPIRED', data.error === 'invalid_grant' ? 'La autorización de Google expiró o fue revocada' : 'No fue posible completar la autorización de Google', 401); return data; }
  private async mutate(connectionId: string, key: string, operation: string, callback: () => Promise<any>) { if (!key) throw new BadRequestException('Idempotency-Key es obligatorio'); const old = await this.db.calendarMutation.findUnique({ where: { connectionId_idempotencyKey: { connectionId, idempotencyKey: key } } }); if (old?.status === 'SUCCEEDED') return old.result; if (old?.status === 'PROCESSING') throw new ConflictException('La operación ya está en proceso'); const row = old ? await this.db.calendarMutation.update({ where: { id: old.id }, data: { status: 'PROCESSING' } }) : await this.db.calendarMutation.create({ data: { connectionId, idempotencyKey: key, operation } }); try { const result = await callback(); await this.db.calendarMutation.update({ where: { id: row.id }, data: { status: 'SUCCEEDED', result: result as Prisma.InputJsonValue } }); return result; } catch (error) { await this.db.calendarMutation.update({ where: { id: row.id }, data: { status: 'FAILED', errorCode: this.errorCode(error) } }); throw error; } }
