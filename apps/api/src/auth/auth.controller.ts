...
@Controller('auth') export class AuthController {constructor(private readonly auth:AuthService){}
 @Public() @Post('login') async login(@Body(new ZodPipe(loginSchema)) body:any,@Req() req:any,@Res({passthrough:true}) res:Response){const result=await this.auth.login(body.email,body.password,context(req));const secure=process.env.NODE_ENV==='production';const domain=process.env.COOKIE_DOMAIN?.trim()||undefined;res.cookie(SESSION_COOKIE,result.token,{httpOnly:true,secure,sameSite:'strict',path:'/',maxAge:12*3600000});res.cookie(CSRF_COOKIE,result.csrf,{httpOnly:false,secure,sameSite:'strict',path:'/',domain,maxAge:12*3600000});return {user:result.user};}
 @Post('logout') @HttpCode(204) async logout(@Req() req:any,@Res({passthrough:true}) res:Response){await this.auth.logout(req.auth.sessionId,req.auth.user.id,context(req));const secure=process.env.NODE_ENV==='production';const domain=process.env.COOKIE_DOMAIN?.trim()||undefined;res.clearCookie(SESSION_COOKIE,{path:'/',secure,sameSite:'strict'});res.clearCookie(CSRF_COOKIE,{path:'/',secure,sameSite:'strict',domain});}
