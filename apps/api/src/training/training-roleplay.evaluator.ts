...
const patterns: Record<Exclude<TrainingCriterion, 'cumplimiento'>, RegExp[]> = {
  ...
  diagnostico: [
    /la prioridad es/i,
    /la brecha/i,
    /necesidad principal/i,
    /antes de revisar opciones/i,
  ],
  conexion_riesgo_impacto: [
    /impacto/i,
    /si eso ocurriera/i,
  ...
  propuesta_valor: [
    /con base en lo que/i,
    /opción podría/i,
    /revisar alternativas/i,
    /sin asumir/i,
  ],
  objeciones: [
    /entiendo .* preocupa/i,
    /¿qué parte/i,
    /¿con qué lo comparas/i,
    /tiene sentido/i,
    /aclarar/i,
  ],
  ...
  empatia: [
    /entiendo/i,
    /gracias por (?:compartir|tu tiempo)/i,
    /es válido/i,
    /sin presión/i,
    /respeto/i,
  ],
  claridad: [
    ...
    /en resumen/i,
    /depende de/i,
    /no tengo información suficiente/i,
  ],
};
...
const improvements: Record<TrainingCriterion, string> = {
  ...
  cumplimiento:
    'Distingue evidencia, simulación y condiciones contractuales; abstente si falta fuente.',
...
const criticalRules: Array<{ code: string; pattern: RegExp; reason: string }> = [
  {
    ...
    pattern: /(?:te|le)?\s*garantizo.{0,40}(?:rentabilidad|rendimiento|tasa)/i,
    reason: 'Garantiza un resultado financiero no soportado.',
  },
  {
    code: 'GUARANTEED_ISSUANCE',
    pattern: /(?:te|le)?\s*(?:garantizo|aseguro).{0,30}(?:aprobaci[oó]n|emisi[oó]n|te aprueban)/i,
    reason: 'Garantiza aprobación o emisión.',
  },
  {
    code: 'GUARANTEED_CLAIM',
    pattern: /(?:te|le)?\s*(?:garantizo|aseguro).{0,35}(?:pago|siniestro|reclamaci[oó]n)/i,
    reason: 'Garantiza el pago de una reclamación.',
  },
  {
    code: 'INVENTED_TERMS',
    pattern: /(?:la prima es|cubre autom[aá]ticamente|la tasa es|el valor asegurado es)\s*[\d$]/i,
    reason: 'Afirma cifras o condiciones sin fuente.',
  },
  {
    code: 'IGNORE_NO_CONTACT',
    pattern: /(?:aunque no quieras|voy a seguir|igual te contactar[eé]|debes escucharme)/i,
    reason: 'Insiste después de una negativa o retiro de contacto.',
  },
  {
    code: 'PROMPT_INJECTION',
    pattern:
      /ignora (?:las|tus) (?:reglas|instrucciones)|system prompt|mu[eé]strame .*clientes?|llama (?:al )?crm|ejecuta .*automat/i,
    reason: 'Intenta alterar instrucciones o acceder a sistemas externos.',
  },
];
...
function complianceFlags(transcript: readonly TrainingTurn[]) {
  ...
  transcript.forEach((turn, index) => {
    ...
    if (/otra (?:aseguradora|compañía).*(?:miente|estafa|nunca paga)/i.test(turn.content))
      flags.push({
        code: 'UNSUPPORTED_DISPARAGEMENT',
        severity: 'HIGH',
        turn: index + 1,
        reason: 'Desacredita a un tercero sin evidencia.',
      });
    if (/dame (?:tu )?(?:contraseña|clave|historia clínica completa|token)/i.test(turn.content))
      flags.push({
        ...
        severity: 'HIGH',
        turn: index + 1,
        reason: 'Solicita información sensible innecesaria.',
      });
  });
  return flags;
}
...
function evidenceFor(
  criterion: Exclude<TrainingCriterion, 'cumplimiento'>,
  ...
  transcript.forEach((turn, index) => {
    ...
    if (patterns[criterion].some((pattern) => pattern.test(turn.content)))
      evidence.push({
        turn: index + 1,
        quoteOrSummary: safeExcerpt(turn.content),
        reason: `Evidencia de ${criterion.replaceAll('_', ' ')}.`,
      });
  });
...
function prematureSolution(transcript: readonly TrainingTurn[]) {
  ...
  const firstTwo = consultantTurns
    .slice(0, 2)
    .map((turn) => turn.content)
    .join(' ');
  return (
    /(?:producto|póliza|plan|solución|cobertura|te recomiendo)/i.test(firstTwo) &&
...
export function evaluateTrainingTranscript(
  ...
  const consultantTurns = transcript.filter((turn) => turn.role === 'CONSULTANT');
  const relevantQuestions = consultantTurns.filter(
    (turn) =>
      /\?/.test(turn.content) &&
      /(?:qué|cómo|cuál|quién|cuándo|preocupa|prioridad|impacto|significaría)/i.test(turn.content),
  ).length;
  ...
  const rubric: CriterionResult[] = TRAINING_RUBRIC.map((criterion) => {
    if (criterion === 'cumplimiento') {
      const evidence = flags.map((flag) => ({
        turn: flag.turn,
        quoteOrSummary: flag.code,
        reason: flag.reason,
      }));
      ...
      return {
        ...
        reason: critical
          ...
          : flags.length
            ? 'Se detectó riesgo de cumplimiento.'
            : 'No se detectaron afirmaciones críticas no soportadas.',
        improvement: improvements[criterion],
      };
    }
    const evidence = evidenceFor(criterion, transcript);
    let score = Math.min(5, evidence.length === 0 ? 0 : 2 + evidence.length);
    if (criterion === 'calidad_preguntas')
      score = Math.min(5, Math.max(0, relevantQuestions - duplicateQuestions));
    if (['diagnostico', 'propuesta_valor'].includes(criterion) && earlySolution)
    ...
    return {
      ...
      reason: evidence.length
        ? `${evidence.length} evidencia(s) relevante(s) en el transcript.`
        : 'No existe evidencia suficiente en el transcript.',
      improvement: improvements[criterion],
    };
  });
  ...
  const score = Number(
    (
      (rubric.reduce((sum, item) => sum + item.score, 0) / (TRAINING_RUBRIC.length * 5)) *
      100
    ).toFixed(2),
  );
  ...
  const recommendationBySkill: Record<string, string> = {
    apertura: 'opening_short_time',
    acuerdo_inicial: 'opening_receptive',
    descubrimiento: 'discovery_family',
    calidad_preguntas: 'discovery_education',
    escucha: 'close_indecisive',
    profundizacion: 'discovery_key_partner',
    diagnostico: 'discovery_retirement',
    conexion_riesgo_impacto: 'discovery_accident',
    propuesta_valor: 'discovery_wealth',
    objeciones: 'objection_price',
    cierre: 'close_next_step',
    siguiente_paso: 'close_appointment',
    cumplimiento: 'compliance_document_conflict',
    empatia: 'discovery_critical_illness',
  ...
  return {
    ...
    rubric,
    compliance: {
      score: rubric.find((item) => item.criterion === 'cumplimiento')!.score,
      critical,
      flags,
    },
    feedback: {
      strengths,
      opportunities,
      missedKeyMoment: earlySolution
        ? 'Se presentó una solución antes de completar el diagnóstico.'
        : (opportunities[0]?.improvement ??
          'No existe evidencia suficiente para identificar un momento perdido.'),
      bestQuestion: scenario?.objective.includes('impacto')
        ? 'Si esa situación no cambia, ¿qué impacto tendría para ti?'
        : '¿Qué es lo más importante que deberíamos comprender antes de revisar alternativas?',
      complianceRisk: flags.length
        ? flags.map((flag) => flag.reason)
        : ['No se detectó riesgo crítico en la evidencia disponible.'],
      recommendedScenarioKey:
        recommendationBySkill[weakest] ?? scenario?.scenarioKey ?? 'discovery_family',
    },
    signals: {
      relevantQuestions,
      duplicateQuestions,
      prematureSolution: earlySolution,
      consultantTurns: consultantTurns.length,
    },
    disclaimer:
      'TRAINING / SIMULATION. No corresponde a CRM, clientes ni condiciones contractuales.',
  };
}

export function evaluateScenarioTranscript(
  scenarioKey: string,
  transcript: readonly TrainingTurn[],
) {
