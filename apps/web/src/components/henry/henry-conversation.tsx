...
export function HenryConversation({
  ...
  const [voiceState, setVoiceState] = useState<'idle' | 'listening' | 'processing' | 'speaking'>(
    'idle',
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      getHenryConversation(restored, internal)
        .then(({ data }) => {
          setSession(restored);
          setMessages(data.messages);
          setConsent(true);
          setEscalated(data.status === 'WAITING_HUMAN');
        })
  ...
  async function submitMessage(active: HenrySession, content: string, current = messages) {
    ...
    setMessages([...current, optimistic]);
    setDraft('');
    ...
    try {
      ...
      setMessages((items) => [
        ...items.map((item) => (item.id === messageId ? { ...item, status: 'COMPLETED' } : item)),
  ...
  async function toggleVoice() {
    ...
    if (
      ...
      setError(
        session
          ? 'El navegador no permite usar el micrófono. Puede continuar por texto.'
          : 'Inicie la conversación antes de usar el micrófono.',
      );
      return;
    }
    try {
      ...
      next.onstop = async () => {
        ...
        try {
          const { data } = await sendHenryVoice(
            session,
            audio,
            Date.now() - recordingStarted.current,
            context,
            internal,
          );
          const voiceUser: HenryMessage = {
            id: crypto.randomUUID(),
            role: 'USER',
            content: data.transcript.transcript,
            status: 'COMPLETED',
            createdAt: new Date().toISOString(),
          };
          setMessages((items) => [...items, voiceUser, ...(data.message ? [data.message] : [])]);
          if (data.message) {
            const spoken = await getHenrySpeech(
              session,
              data.message.id,
              data.voiceSessionId,
              internal,
  ...
  if (loading)
    return (
      <div className="henry-loading">
        <LoaderCircle className="animate-spin" />
        <span>Recuperando conversación segura…</span>
      </div>
    );
  const console = (
    <section
      className={`henry-console ${variant === 'global' ? 'is-global' : ''}`}
      aria-label={internal ? 'Copiloto Henry' : 'Conversación con Henry'}
    >
      <header>
        <div className="henry-presence">
          <span>H</span>
          <div>
            <strong>Henry</strong>
            <small>
              {internal
                ? 'Copiloto comercial · contexto autorizado'
                : 'Asistente virtual · Canal web'}
            </small>
          </div>
        </div>
      ...
      {!session ? (
        <div className="henry-consent">
          <div>
            <LockKeyhole />
            <p>
              <strong>{internal ? 'Copiloto contextual' : 'Antes de comenzar'}</strong>
              <span>
                {internal
                  ? 'Henry consulta únicamente información permitida por su rol y la página activa.'
                  : 'La conversación será persistida para orientar su solicitud. Henry no reemplaza la asesoría humana.'}
              </span>
            </p>
          </div>
          {!internal && (
      ...
      {session && (
        <footer>
          {escalated ? (
            <p className="henry-escalated">
              <Check /> Solicitud humana registrada.
            </p>
          ) : (
            !internal && (
              <button
                type="button"
                className="henry-human"
          ...
          <form onSubmit={send}>
            <label htmlFor={`henry-message-${variant}`} className="sr-only">
              Mensaje para Henry
            </label>
            <textarea
              id={`henry-message-${variant}`}
              value={draft}
              onChange={(event) => {
              ...
              placeholder={
                internal
                  ? 'Pregunte sobre el contexto actual…'
                  : 'Cuénteme qué le gustaría resolver…'
              }
              disabled={sending}
            />
            <button
              type="button"
              className={`henry-microphone is-${voiceState}`}
              aria-label={
                voiceState === 'listening'
                  ? 'Detener grabación'
                  : voiceState === 'speaking'
                    ? 'Detener voz de Henry'
                    : 'Hablar con Henry'
              }
              aria-pressed={voiceState === 'listening'}
              onClick={() => void toggleVoice()}
              disabled={sending && voiceState !== 'listening'}
            >
              {voiceState === 'listening' ? (
                <Square />
              ) : voiceState === 'speaking' ? (
            ...
            <button aria-label="Enviar mensaje" disabled={!draft.trim() || sending}>
              <Send />
            </button>
          </form>
          {voiceState !== 'idle' && (
            <p className="henry-voice-status" role="status">
              <Volume2 />{' '}
              {voiceState === 'listening'
                ? 'Escuchando. Pulse de nuevo para enviar.'
                : voiceState === 'processing'
                  ? 'Transcribiendo y consultando a Henry…'
                  : 'Henry está respondiendo por voz.'}
            </p>
          )}
          <small>
            {internal
              ? 'El acceso a CRM se resuelve en servidor según su rol.'
              : 'No comparta contraseñas ni información financiera sensible.'}
          </small>
        </footer>
      )}
    </section>
  ...
  return (
    <div className="henry-stage">
      <section className="henry-introduction" aria-labelledby="henry-main-title">
        ...
        <h1 id="henry-main-title">
          Una conversación que empieza por <em>comprender.</em>
        </h1>
        <p>
          Henry es el asistente virtual de HAVONA CAPITAL GROUP. Conserva el contexto autorizado y
          conecta cada conversación con nuestro equipo.
        </p>
        <div className="henry-principles">
          <span>
            <Check /> Claro
          </span>
          <span>
        ...
        <div className="henry-orbit" aria-hidden="true">
          <i />
          <i />
          <i />
          <span>H</span>
