        ...
        <section className="rounded-[24px] border bg-white p-5" aria-labelledby="roleplay-mode">
          <div className="flex items-center justify-between gap-4 border-b pb-4">
            <div>
              <p
                id="roleplay-mode"
                className="text-xs font-bold uppercase tracking-widest text-brand-700"
              >
              ...
              <li
                ...
                className={
                  turn.role === 'CONSULTANT'
                    ? 'ml-10 rounded-2xl bg-brand-900 p-4 text-white'
                    : 'mr-10 rounded-2xl bg-slate-100 p-4 text-slate-800'
                }
              >
                <span className="text-xs font-bold uppercase tracking-wide">
                  {turn.role === 'CONSULTANT' ? 'Tú' : 'Prospecto'}
                </span>
          ...
          <form onSubmit={(event) => void send(event)} className="flex gap-2">
            <label htmlFor="roleplay-message" className="sr-only">
              Tu respuesta
            </label>
            <input
              id="roleplay-message"
              ...
              placeholder="Formula una pregunta o responde al prospecto"
            />
            <button
              disabled={busy || !message.trim()}
              className="rounded-full bg-brand-900 px-5 text-white disabled:opacity-50"
            >
              Enviar
            </button>
          </form>
        ...
        <section className="space-y-6" aria-labelledby="coach-mode">
          <div className="rounded-[24px] border bg-white p-6">
            <p
              ...
              className="text-xs font-bold uppercase tracking-widest text-brand-700"
            >
              Coach mode · Evaluación final
            </p>
            <p className="mt-2 font-display text-5xl text-brand-950">{evaluation.score}/100</p>
            {evaluation.compliance.critical && (
              <Alert>Se detectó un riesgo crítico de cumplimiento.</Alert>
            )}
          </div>
          <div className="grid gap-5 md:grid-cols-2">
            <div className="rounded-2xl border bg-white p-5">
              ...
              <ul className="mt-3 space-y-2">
                {evaluation.feedback.strengths.map((item) => (
                  <li key={item.criterion}>
                    {item.criterion.replaceAll('_', ' ')} · {item.score}/5
                  </li>
                ))}
              </ul>
            </div>
            <div className="rounded-2xl border bg-white p-5">
              ...
              <ul className="mt-3 space-y-3">
                {evaluation.feedback.opportunities.map((item) => (
                  <li key={item.criterion}>
                    <strong>{item.criterion.replaceAll('_', ' ')}</strong>
                    <p className="text-sm text-slate-600">{item.improvement}</p>
                  </li>
                ))}
              </ul>
            </div>
