...
vi.mock('@/lib/calendar', () => ({
  calendarApi: {
    ...
    events: vi.fn(), rules: vi.fn(), connect: vi.fn(), disconnect: vi.fn(), updateRules: vi.fn(), create: vi.fn(), sync: vi.fn(),
  },
}));
...
describe('AgendaPage', () => {
  ...
  it('no simula eventos y ofrece OAuth real cuando está desconectada', async () => {
    render(<AgendaPage />);
    await waitFor(() => expect(screen.getByRole('button', { name: /conectar google calendar/i })).toBeInTheDocument());
